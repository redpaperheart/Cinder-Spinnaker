...
SpinnakerCapture::SpinnakerCapture(){
	...
	mResY = 1536;
	//mImageCnt = 0;
	//mTotalFramesToSave = 200;
	//mSavedSurfaceRefVector.resize(mTotalFramesToSave);
	//mCamSurfaceRef = ci::Surface::create(mResX, mResY, false, ci::SurfaceChannelOrder::RGBA);
...
void SpinnakerCapture::setup(int index){
	...
	//setup the camera index
	mCam = mCamList.GetByIndex(mCamIndex);
	mCam->Init();
	...
	mCam->AcquisitionFrameRate.SetValue(30);

	mCam->BeginAcquisition();
	lastFrame = getElapsedSeconds();

	//try to reduce buffer stream and try to send newest out first
...
//void SpinnakerCapture::loadTextureThreadFn(ci::gl::ContextRef sharedGlContext) {
//}
...
void SpinnakerCapture::update(){
	...
	//if (limitFPS && getElapsedSeconds()< lastFrame + (1.0f /fps ) ) {
	//	//ignore
	//	return;
	//}
	//take the latest surface and copy to a texture
	//if (isFrameAvailable()) {
	...
	//lastFrame = getElapsedSeconds();
	//int imgQueueCount = mCam->TransferQueueCurrentBlockCount();
	//int imgQueueMax = mCam->TransferQueueMaxBlockCount();// = 1;
	//imgQueueMax = 1;
	//mCam->tran
	//int imgQueueCount = mCam->GetNextImage();
	//CI_LOG_I("imgQueueMax: " << mCam->TransferQueueMaxBlockCount());
...
int SpinnakerCapture::AcquireImages(CameraPtr pCam, INodeMap & nodeMap, INodeMap & nodeMapTLDevice) {
	...
	try {
		...
		if (mCaptureCount<10 && !nextImg->IsIncomplete()) {
...
void SpinnakerCapture::draw(){
	//ci::gl::draw( mCamTexture, ci::Rectf(ci::vec2(0.0), ci::vec2(mResX, mResY)) );
